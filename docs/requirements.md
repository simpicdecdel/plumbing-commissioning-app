...
- Implementation state: v0.4.8 and its production backend were deployed on 9 September 2026. Physical production acceptance on 10 September confirmed assignment withdrawal, retained-edit recovery, forced password change, Technician console denial and online deactivation, but found that an offline restart could restore stale Technician membership after deactivation. v0.4.9 makes cached access fail closed. It passed local and isolated live coverage, was deployed on 10 September, and passed the repeated physical-iPhone offline close/reopen check with the deactivated account still blocked. See `admin-console.md` and `release-verification.md`.
