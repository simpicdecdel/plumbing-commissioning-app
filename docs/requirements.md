...
- Implementation state: v0.4.8 is merged, and the production migration, single-operator bootstrap, Edge Function and GitHub Pages client were confirmed deployed on 9 September 2026. Local, isolated live and deployed test-handler checks passed. Physical production acceptance on 10 September confirmed assignment withdrawal, retained-edit recovery, forced password change, Technician console denial and online deactivation, but found that an offline restart could restore stale Technician membership after deactivation. The v0.4.9 candidate makes cached access fail closed; deployment and physical-iPhone retesting remain pending. See `admin-console.md` and `release-verification.md`.
