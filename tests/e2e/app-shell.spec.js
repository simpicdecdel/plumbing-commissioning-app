...
test('shows the current release without horizontal overflow', async ({ page }) => {
  ...
  await expect(page.getByText('v0.4.8', { exact: true })).toBeVisible();
...
test('serves a valid installable shell', async ({ request }) => {
  ...
  expect(workerSource).toContain("plumbing-commissioning-v0.4.8-admin-console");
  expect(workerSource).toContain("'./storage.js?v=0.4.8-admin-console'");
  expect(workerSource).toContain("'./sync.js?v=0.4.8-admin-console'");
  expect(workerSource).toContain("'./vendor/dexie.min.js?v=0.4.8-admin-console'");
  expect(workerSource).toContain("'./vendor/remote-client.min.js?v=0.4.8-admin-console'");
...
test('registers a service worker that controls the application shell and caches public config', async ({ page }) => {
  ...
  const cachedConfig = await page.evaluate(async () => {
    ...
    const response = await (await caches.open(cacheName)).match('./config.js?v=0.4.8-admin-console');
